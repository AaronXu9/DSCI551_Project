...
def mkdir(url):
    requests.put(url, '{"0":0}')

def ls(url):
    response = requests.get(url)
    print(*list(response.json().keys())[1:], sep=" ")
...
def put(file, url_meta, url_partition , k):
    ...
    for i in range(1,number_lines,partition_size):
        ...
        df = pd.read_csv(in_csv,names=df_header,nrows = partition_size,skiprows = i,usecols=['genres','id','original_language','title','production_countries','release_date','vote_average','popularity'])
        data = df.to_json(orient='records')
        requests.put(to_url,data)
        partition_meta = '{{"{0}" : "{1}"}}'.format("p"+str(n),to_url)
        requests.patch(url_meta + file_dir +'/partitions.json',str(partition_meta))
...
def readPartition(file, partition_num):
    ...
    return requests.get(partition).json()
